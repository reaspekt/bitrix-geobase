...
<script language="JavaScript">
function updateCoreSumbit() {
    let submitDialog = new BX.CDialog({
        title: "<?=Loc::getMessage("REASPEKT_UPDATE_CORE")?>",
        content: '<?=Loc::getMessage("REASPEKT_UPDATE_CORE_SUBMIT_TEXT")?>',
        icon: 'head-block',
        resizable: true,
        draggable: true,
        height: '168',
        width: '526',
        buttons: ['<input type="button" onclick="updateCore()" value="<?=Loc::getMessage("REASPEKT_NEW_CORE_BTN")?>">', BX.CDialog.btnClose]
    });
    submitDialog.Show();
}
function updateCore() {
    BX.ajax.runAction('reaspekt:geobase.admin.updateCore', {
        data: {}
    }).then(function(response) {
        console.log(response);
        location.reload();
...
<form method='POST' action='<?echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialcharsbx($request['mid'])?>&amp;lang=<?=$request['lang']?>' name='reaspekt_geobase_settings'><?
    ...
    $tabControl->BeginNextTab();
        ...
        <tr class="heading">
            <td colspan="2"><?=Loc::getMessage("REASPEKT_NEW_CORE_HEADING")?></td>
        </tr>
        <?
        if (
                ...
                <td colspan="2">
                    <div style="text-align: center">
                        <div class="adm-info-message" style="display:block">
                            <?=Loc::getMessage("REASPEKT_NEW_CORE_TXT")?>
                        </div>
                        <input type="button" onclick="updateCoreSumbit()" value="<?=Loc::getMessage("REASPEKT_NEW_CORE_BTN")?>">
                    </div>
                </td>
            </tr>
        <?
            ...
            <tr>
                <td colspan="2">
                    <div style="text-align: center">
                        <div class="adm-info-message" style="display:block">
                            <?=Loc::getMessage("REASPEKT_OLD_ENVIRONMENT")?>
                        </div>
                        <input disabled="disabled" type="button" value="<?=Loc::getMessage("REASPEKT_NEW_CORE_BTN")?>">
                    </div>
                </td>
            </tr>
            <?
